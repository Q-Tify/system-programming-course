...
struct coro {
	...
	//To store program counter, register values, and stack pointer
	//When we switch between coroutines we need to save this
	sigjmp_buf ctx;

	/** True, if the coroutine has finished. */
	bool is_finished;
	
	//How many times coroutine switched the context(program counter, register values and etc.)
...
{	
	//From currently working coroutine
	struct coro *from = coro_this_ptr;

	//Add +1 to the current count coroutine
	++from->switch_count;

	//0 means that we don't need to capture a signal mask to determine whatever 
	//it is the initial function call or jump back
	if (sigsetjmp(from->ctx, 0) == 0)
	//1 means that it will be the return value of the sigsetjmp call that corresponds to the saved context
		siglongjmp(to->ctx, 1);
...
coro_yield(void)
{
	...
	if (to == NULL)
		//Choose coroutine to start from dead ones
